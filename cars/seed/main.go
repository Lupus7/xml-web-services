...
func main() {
	...
	defer store.Close()
	fmt.Println("Connection with database open..")
...
func populateDatabase(store *postgres.Store) error {
	...
	for _, fuel := range fuels {
		err := store.DB().Create(&fuel).Error
		if err != nil {
			return err
		}
	}

	transms := []model.Transmission{}
	transms = append(transms, manual, automatic)
	for _, trans := range transms {
		err := store.DB().Create(&trans).Error
		if err != nil {
			return err
		}
	}
	...
	classes := []model.Class{}
	classes = append(classes, economic, luxury, sporsts, Suv, oldTimer)
	for _, class := range classes {
		err := store.DB().Create(&class).Error
		if err != nil {
			return err
		}
	}
